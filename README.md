...
Formally MARS uses the positive portion of what are called hinge functions to separate the data at previously mentioned spline knots. In mathematical notation:
{█(X-t,        if X>t@       0,           otherwise)┤    and	 {█(t-X,        if X<t@       0,           otherwise)┤
Where X are the data and t are the proposed knot location within the data. 
